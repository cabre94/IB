...
PATH_FOLDER = os.path.join(SAVE_FOLDER, "4_targeted")
...
def step(adversarial, min_pert, max_pert, lr):
    ...
    prob = model.predict(adversarial)[0, pizza_index]

    random_pert = initialize_randomTensor(adversarial, 0.6)

    new_adv = adversarial + random_pert * lr
    new_adv = tf.clip_by_value(new_adv, min_pert, max_pert)
    new_adv = tf.clip_by_value(new_adv, -1, 1)

    new_prob = model.predict(new_adv)[0, pizza_index]
...
def generate_adversarial_example(adversarial, epsilon=1, lr=1e-2):
    ...
    # Iteramos hasta obtener una probabilidad de 0.6
    while prob < 0.6:
...
adver = generate_adversarial_example(imagen, epsilon=1, lr=1e-2)
pert = imagen - adver
