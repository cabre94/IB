...
void trieGetEnabledKeys(const Trie *trie, const char *prefix, bool enabledKeys[ALPHABET_SIZE]){
	TrieNode* pCursor = trie->searchPrefix(prefix);
	...
	if(pCursor){
		for(int i=0; i < ALPHABET_SIZE; i++){
			if(pCursor->children[i])
				enabledKeys[i] = true;
		}
	}
	// Si no existe hay que oner todo en false y listo
	// if(pCursor == nullptr)
	// 	throw string("No existe el prefix " + string(prefix));
}

// TODO
using ListWord = list<string>;

void pushKeysToList(TrieNode* tn, const string& prefix, ListWord& l){
	if(tn->defineKey)
		l.push_back(prefix);
	
	for(int i=0; i < ALPHABET_SIZE; i++){
		if(tn->children[i]){
			pushKeysToList(tn->children[i], prefix + getCharForIndex(i), l);
		}
	}
...
ListWord trieGetKeys(const Trie *trie, const char *prefix){
	...
	if(pCursor){
		pushKeysToList(pCursor, string(prefix), keys);
	}
